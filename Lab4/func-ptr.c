...
{
	const Process *proc_this = (const Process *)this;
  const Process *proc_that = (const Process *)that;

  if (proc_this -> arrival_time < proc_that -> arrival_time) {
    return -1;
  }
  else if (proc_this -> arrival_time > proc_that -> arrival_time){
    return 1;
  }
  else {
    if(proc_this -> priority < proc_that -> priority) {
      return -1;
    }
    else if (proc_this -> priority > proc_that -> priority) {
      return 1;
    }
    else {
      if (proc_this -> pid < proc_that -> pid) {
        return -1;
      }
      else if (proc_this -> pid > proc_that -> pid) [
        return 1;
      ]
      else {
        return 0;
